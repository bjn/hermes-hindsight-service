...
async def acquire_with_retry(backend_or_pool: Any, max_retries: int = DEFAULT_MAX_RETRIES) -> AsyncIterator[Any]:
    ...
    if isinstance(backend_or_pool, DatabaseBackend) or getattr(backend_or_pool, "_wraps_backend", False):
        # Use the backend's acquire context manager with retry.  Keep the retry
        # boundary strictly around __aenter__(); exceptions raised by the caller
        # while using the yielded connection must propagate normally and must not
        # be mistaken for acquisition failures.  Catching the whole yield block
        # here causes contextlib to raise "generator didn't stop after athrow()"
        # when caller code times out or is cancelled.
        start = time.time()
        last_exception = None
        conn_cm = None
        conn = None
        for attempt in range(max_retries + 1):
            conn_cm = backend_or_pool.acquire()
            try:
        ...
        try:
            ...
            suppress = await conn_cm.__aexit__(type(exc), exc, exc.__traceback__)
            if not suppress:
                raise
        else:
            assert conn_cm is not None
